CKEDITOR.dialog.add('pdfExtractTextDialog', function(editor) {
    return {
        ...
        buttons: [{
            ...
            onClick: function() {
                const dialog = this.getDialog();
                const extractedText = document.getElementById('resultText').value;
                if (extractedText) {
                    editor.insertHtml('<div style="white-space: pre-wrap; font-family: monospace;">' + extractedText + '</div>');
                    dialog.hide();
                } else {
                    alert('삽입할 내용이 없습니다.');
                }
...
PdfFileProcessor.prototype.processFile = function() {
    ...
    this.extractTextFromPdf(validation.file)
        .then(function(result) { // result now contains { textContent, metadata }
            self.handleResult(result.textContent, validation.file.name, validation.file.size, validation.file.lastModifiedDate, result.metadata);
        })
        ...
        reader.onload = function(e) {
            ...
            pdfjsLib.getDocument({ data: arrayBuffer }).promise.then(function(pdf) {
                ...
                pdf.getMetadata().then(function(data) {
                    metadata = data.info || {}; // Basic info
                    if (data.metadata) { // XMP metadata
                    ...
                    // Continue with text extraction after metadata attempt
                    if (numPages === 0) { // Handle empty PDF
...
PdfFileProcessor.prototype.handleResult = function(text, fileName, fileSize, fileLastModifiedDate, pdfMetadata) {
    const resultText = document.getElementById('resultText');
    const resultJson = document.getElementById('resultJson');
    const viewerTabs = document.getElementById('viewerTabs');

    // Format creation date to yyyy-mm-dd:hh-mm-ss Seoul time
    const date = new Date(fileLastModifiedDate);
    const options = {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
    ...
    const formattedDate = date.toLocaleString('ko-KR', options).replace(/\. /g, '-').replace(/\.$/, '').replace(/ /g, ':');

    const jsonOutput = JSON.stringify({
        fileName: fileName,
        fileSize: fileSize,
        creationDate: formattedDate, // Using lastModifiedDate as creationDate
    ...
    if (resultText && resultJson && viewerTabs) {
        ...
        resultJson.value = jsonOutput;
        viewerTabs.style.display = 'block';
        showTab('text'); // Show text tab by default
    }
    ...
    this.dialog.extractedFileName = fileName; // Store file name for download
};
...
PdfFileProcessor.prototype.showLoading = function(show) {
    ...
    const viewerTabs = document.getElementById('viewerTabs'); // Use viewerTabs instead of resultContent
    ...
    if (viewerTabs) viewerTabs.style.display = show ? 'none' : 'block'; // Hide viewerTabs during loading
